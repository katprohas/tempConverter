...
namespace tempConverter
{
    class temps
    {
        ...
        private static int convertTemp(char type, int NumToConvert)
        {
            int convertedValue = 0, numToConvert = NumToConvert;
            if (type == 'F')
            {
        ...
        private static double convertTemp(char type, double NumToConvert)
        {
            double convertedValue = 0, numToConvert = NumToConvert;
            if (type == 'F')
            {
                convertedValue = (numToConvert * 9) / 5 + 32;
                Console.WriteLine($"{numToConvert} degrees celsius converted to fahrenheit is {convertedValue}");
